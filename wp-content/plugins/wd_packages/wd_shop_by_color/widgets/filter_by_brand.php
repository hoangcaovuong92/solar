...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	if (!class_exists('tvlgiao_wpdance_widget_product_brand_filter')) {
		class tvlgiao_wpdance_widget_product_brand_filter extends WP_Widget {
			...
			function __construct() {
				...
				/* Create the widget. */
				parent::__construct( 'wd_widget_product_filter_by_brand', $this->wpdance_widget_name, $widget_ops );
			}
			...
			function widget( $args, $instance ) {
				...
				if ( count( $terms ) > 0 ) {
					...
					// List display
					echo '<ul class="wd-filter-by-brand-list">';
