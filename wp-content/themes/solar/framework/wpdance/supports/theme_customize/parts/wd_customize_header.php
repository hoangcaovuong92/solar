...
$wp_customize->add_section( 'tvlgiao_wpdance_header_config' , array(
		'title'       		=> esc_html__( 'WD - Header', 'laparis' ),
		'description' 		=> esc_html__( 'Custom header site.' , 'laparis' ),
		'priority'    		=> 505,
	));
	//---------------------------------------------------------------//
...
$wp_customize->add_control( 'tvlgiao_wpdance_header_show_logo_title_control', array(
	...
	'description'   	=> esc_html__( 'This setting is only visible to the default header template.', 'laparis' ),
	'type'    			=> 'select',
	'choices' 			=> array(
		'0'				=> "Show Logo",
		'1'				=> "Show Site Title",
...
$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'tvlgiao_wpdance_header_favicon', array(
    'label'    			=> esc_html__( 'Favicon', 'laparis' ),
    'section'  			=> 'tvlgiao_wpdance_header_config',
    'settings' 			=> 'tvlgiao_wpdance_header_favicon',
    'description'   	=> esc_html__( 'Favicon is the little icon that browsers display next to a page\'s title on a browser tab, or in the address bar next to its URL.', 'laparis' ),
)));
...
$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'tvlgiao_wpdance_header_logo_url', array(
    ...
    'description'   	=> esc_html__( 'Header logo is only visible to the default header template.', 'laparis' ),
)));
