	...
	if( !class_exists( 'tvlgiao_wpdance_widget_user_link' ) ) {
		class tvlgiao_wpdance_widget_user_link extends WP_Widget{
		    function __construct() {
				$widget_ops 		= array('classname' => 'widget_user_link', 'description' => esc_html__('User Link Widget','wd_package'));
				$control_ops 		= array('width' => 400, 'height' => 350);
				parent::__construct('user_link', esc_html__('WD - User Links','wd_package'), $widget_ops);
			}
		    function form( $instance )
		    {
		        ...
		        ?>
					<p>
						<label for="<?php echo esc_attr( $this->get_field_id('show_icon')); ?>"><?php esc_html_e('Show Title:','wd_package'); ?></label>
						<select class="widefat" name="<?php echo esc_attr( $this->get_field_name('show_icon')); ?>" id="<?php echo esc_attr($this->get_field_id('show_icon')); ?>">
							<option value="1" <?php echo ($show_icon == 1)?'selected':'' ?> ><?php esc_html_e('Yes','wd_package'); ?></option>
							<option value="0" <?php echo ($show_icon == 0)?'selected':'' ?> ><?php esc_html_e('No','wd_package'); ?></option>
						</select>
					</p>
		            <p>
