...
<?php
	 ...
	 */
	$custom_content = '';
	extract(array_merge(tvlgiao_wpdance_get_data_package( 'archive-blog' ), tvlgiao_wpdance_get_taxonomy_custom_content()));
		...
		<!-- Content Index -->
			<div class="wd-default-blog-archive <?php echo esc_attr($content_class); ?> <?php echo esc_attr($layout_class); ?>">
				<?php if ($custom_content && tvlgiao_wpdance_is_visual_composer()): ?>
					<div class="wd-archive-custom-content"><?php echo do_shortcode($custom_content); ?></div>
				<?php endif ?>
				<?php if ( have_posts() ) : ?>
					<!-- Start the Loop --> 
