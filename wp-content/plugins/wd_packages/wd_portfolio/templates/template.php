<?php
if(!function_exists('show_wd_portfolio')){
			...
			<div class="container">
				<div class="row">
					<div class="wrap-filter">
						...
						<div class="filter_portfolio_wp">
							<?php $category_portfolio = get_terms( 'wd-portfolio-category' ); ?>
							<?php $count_category = count( $category_portfolio ); ?>
							...
							foreach ( $category_portfolio as $term ) { ?>
								<a class="<?php if ( $term->name == $category_name ) {
								   ...
								   href="?category_name=<?php echo $term->name; ?>&category_portfolio_id=<?php echo $term->term_id; ?>"><?php echo $term->name; ?><?php if ( $i != $count_category ) {
										echo " /";
									} ?></a>
								<?php $i ++;
							} ?>
			...
			<div id="portfolio-container-holder">
				<div class="portfolio-galleries" id="portfolio-galleries">
					...
					<div id="portfolio-galleries-holder">
						...
						if ( $category_portfolio_id != - 1 ) {
							$args['tax_query'] = array(
								array(
									'taxonomy' => 'wd-portfolio-category',
									'terms'    => $category_portfolio_id,
									'field'    => 'term_id',
								),
							);
						}
						...
						if ( $portfolios->have_posts() ) : while ( $portfolios->have_posts() ) : $portfolios->the_post();
							...
							$div_pos         = $count % 3;
							?>
							<?php $class_span = 'col-sm-' . ( 12 / $columns ); ?>
							<div class="item <?php echo esc_attr( $class_span ); ?> item-portfolio"
							     data-type="<?php echo $term_list; ?>" data-id="<?php echo $post->ID; ?>">
										...
										<div class="thumbnail">
											<div class="thumb-image post-item ">
												...
												<a class="image" href="<?php echo esc_url( $post_url ); ?>">
													<?php if ( $thumburl[0] ): ?>
														<img alt="<?php echo esc_attr( $post_title ); ?>"
														     title="<?php echo esc_attr( $post_title ); ?>"
														     class="opacity_0"
														     src="<?php echo esc_url( $thumburl[0] ); ?>" />
													<?php else: ?>
														<img alt="<?php echo esc_attr( $post_title ); ?>"
														     title="<?php echo esc_attr( $post_title ); ?>"
														     class="opacity_0"
														     src="<?php echo get_template_directory_uri(); ?>/images/720x480.png" />
													<?php endif; ?>
												</a>
					...
					<div class="page_navi">
						<div class="nav-content">
							<div class="wp-pagenavi">
								<?php
								//wp_outline_ew_pagination_portfolio(3, $portfolios);
								wp_reset_postdata();
								?>
							</div>
						</div>
		...
		<script type="text/javascript">
			...
			jQuery( function () {
				...
				applications.find( 'div.item-portfolio' ).each( function ( i, value ) {
					...
					;
					if ( i % <?php echo esc_js( $columns ) ?> == <?php echo esc_js( $columns ) - 1; ?> || i == <?php echo esc_js( $post_count ) - 1; ?> ) {
