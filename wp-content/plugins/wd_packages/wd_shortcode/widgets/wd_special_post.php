...
if( !class_exists( 'tvlgiao_wpdance_widget_special_post' ) ) {
	class tvlgiao_wpdance_widget_special_post extends WP_Widget{
		...
		function widget( $args, $instance ) {
			...
			if ( $recent_posts->have_posts() ) {
				...
				?>
				<div class="wd_widget_recent_posts_wrapper <?php echo ($show_nav)?'has_navi':''; ?> <?php echo ($style_show == 'style-1') ? 'wd-special-post-style-grid' : 'wd-special-post-style-list'; ?>" id="<?php echo esc_attr( $random_id ); ?>">
										...
										<?php if(has_post_thumbnail()){ ?>
											<div class="post_thumbnail image">
												<a class="wd-effect-blog" href="<?php the_permalink(); ?>">
													<?php the_post_thumbnail('wd_image_size_thumbnail',array('class' => 'thumbnail-effect-1', 'alt' => esc_attr(get_the_title()), 'title' => esc_attr(get_the_title()))); ?>
												</a>
											</div>
										<?php } ?>
									</div>
	...
	function wp_widget_register_widget_special_post() {
		register_widget( 'tvlgiao_wpdance_widget_special_post' );
	}
	add_action( 'widgets_init', 'wp_widget_register_widget_special_post' );
}
?>
