...
if (!class_exists('WD_Post_Like')) {
	class WD_Post_Like {
		...
		public function __construct(){
			...
			add_shortcode( 'wd_post_like', array($this, 'add_shortcode' ) );
		}
		
		protected function constant(){
			define('WDPL_BASE'		,   TVLGIAO_WPDANCE_THEME_FUNCTIONS . '/class/post_like'	);
			define('WDPL_BASE_URI'	,   TVLGIAO_WPDANCE_THEME_FUNCTIONS_URI . '/class/post_like');
			define('WDPL_JS'		, 	WDPL_BASE_URI . '/js'		);
			define('WDPL_CSS'		, 	WDPL_BASE_URI . '/css'		);
		}

		
		public function enqueue_scripts() {
			wp_enqueue_style( 'wd-simple-likes-public-style', WDPL_CSS.'/simple-likes-public.css');
		...
		public function get_simple_likes_button( $post_id, $is_comment = NULL ) {
			...
			if ( $is_comment == 1 ) {
				$post_id_class = esc_attr( ' wd-post-like-comment-button-' . $post_id );
				$comment_class = esc_attr( ' wd-post-like-comment' );
				$like_count = get_comment_meta( $post_id, "_comment_like_count", true );
				$like_count = ( isset( $like_count ) && is_numeric( $like_count ) ) ? $like_count : 0;
			} else {
				$post_id_class = esc_attr( ' wd-post-like-button-' . $post_id );
			...
			// Loader
			$loader = '<span id="wd-post-like-loader"></span>';
		...
		/**
		 * Utility public function to format the button count,
		 ...
		 * @since    0.5
		 */
		public function wd_format_count( $number ) {
			$precision = 2;
		...
		public function get_like_count( $like_count ) {
			...
			$count = '<span class="wd-post-like-count">' . $number . '</span>';
			return $count;
		} // get_like_count()
	...
	WD_Post_Like::get_instance();  // Start an instance of the plugin class 
}